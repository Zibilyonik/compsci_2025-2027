...
#looping over a file and counting exercise:
#we need to read data from a text file and store it in a variable
#we need to loop trough da text file and count how many times each letter appears
#we need to check if the letter exists as a key in the dictionary, if it does, add one to the counter
#if it doesn't assign it's value to 1 by ex, mydictionary["b"]=1
...
#Exercise from google classroom:
def shopping():
    purchases = []
    """
...
#𝐭𝐡𝐞 𝐩𝐚𝐫𝐭 𝐈 𝐚𝐝𝐝𝐞𝐝
    with open("purchases.txt", "w") as file:
        file.write("your purchases:\n")
        for item_name, item_cost in purchases:
