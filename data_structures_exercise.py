...
for list in shopping_list:
    if search_for in list:
        count_with_a += 1
print("Items containing the letter ",search_for, ":", count_with_a)
...
steve_total = sum(scores[0])   # TODO: calculate total of scores[0]
steve_average = steve_total/len(scores[0])   # TODO: calculate average
print(f"Steve average: {steve_average:.2f}")
...
for score in scores[0]:
    total += score 
    num_scores += 1
print(f"Steve average: {total/num_scores:.2f}")
...
for score in scores[1]:
    total += score 
    num_scores += 1
print(f"Hwoarang average: {total/num_scores:.2f}")
...
for score in scores[2]:
    total += score 
    num_scores += 1
print(f"Asuka average: {total/num_scores:.2f}")

# TODO: write a loop over scores here that prints each student's average

student_id = 0

while student_id<len(scores):
    total = 0
    num_scores = 0
    for score in scores[student_id]:
        total += score 
        num_scores += 1
    print(f"Student {student_id} average: {total/num_scores:.2f}")
    student_id += 1
# ---------------------------------------------------------------------------
...
def browser_history_demo():
    ...
    push(history_stack, "youtube.com")
    push(history_stack, "gmail.com")
    print(peek(history_stack))
    pop(history_stack)
    print(peek(history_stack))
...
def fill_print_queue():
    ...
    print_queue = []
    valid_document_name = True

    while valid_document_name: 
        doc_name = input("Enter document name for printing or type 'Done' to exit: ")
        valid_document_name = doc_name != "" and doc_name != "Done"
        if valid_document_name: 
            enqueue(print_queue, doc_name)
    return print_queue
...
def process_print_queue(print_queue):
    ...
    while not queue_is_empty(print_queue):
        print("Printing document: ", dequeue(print_queue))

process_print_queue(fill_print_queue())
